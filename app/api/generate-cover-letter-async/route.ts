...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Create async job for cover letter generation
    ...
    console.log(`[ASYNC PROCESSING] Created job ${jobProcessingId} for cover letter generation`);
    ...
    return NextResponse.json({
      ...
      message: 'Cover letter generation started. You will be notified when it\'s ready.',
      status: 'processing'
    });

  } catch (error) {
    console.error('[ASYNC COVER LETTER GENERATION] Error:', error);
    
    return NextResponse.json({
      error: error instanceof Error ? error.message : 'Failed to start cover letter generation',
